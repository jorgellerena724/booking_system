...
def crear_reserva_desde_importacion(data_reserva, user, fuente):
    """Función FINAL para crear reservas - Compatible con todos los formatos"""
    try:
        # ===== 1. OBTENER BOOKING CODE =====
        booking_voucher = data_reserva.get('voucher') or data_reserva.get('booking_code')
        if not booking_voucher:
            print("❌ Error: No hay booking code/voucher en los datos")
            return False
            
        if Reservation.objects.filter(booking_code=booking_voucher).exists():
            print(f"⚠️ Reserva con voucher {booking_voucher} ya existe")
            return False
        
        # DEBUG: Mostrar qué campos tiene data_reserva
        print(f"🔍 Campos en data_reserva: {list(data_reserva.keys())}")
        
        # ===== 2. PROCESAR NOMBRES DE PASAJEROS - FORMATO PRIORITARIO =====
        nombres_clientes = ""
        lista_pasajeros = []
        
        # PRIORIDAD 1: Lista de pasajeros del parser nuevo (BEDBANKGLOBAL)
        if 'pasajeros' in data_reserva:
            if isinstance(data_reserva['pasajeros'], list):
                lista_pasajeros = data_reserva['pasajeros']
                print(f"✅ PRIORIDAD 1: {len(lista_pasajeros)} pasajeros desde lista 'pasajeros'")
            elif isinstance(data_reserva['pasajeros'], str):
                lista_pasajeros = [data_reserva['pasajeros']]
                print(f"✅ PRIORIDAD 1: 1 pasajero desde string 'pasajeros'")
        
        # PRIORIDAD 2: Nombres concatenados
        elif 'nombres_completos' in data_reserva and data_reserva['nombres_completos']:
            nombres_completos = data_reserva['nombres_completos']
            # Dividir por comas para crear lista
            lista_pasajeros = [n.strip() for n in nombres_completos.split(',') if n.strip()]
            print(f"✅ PRIORIDAD 2: {len(lista_pasajeros)} pasajeros desde 'nombres_completos'")
        
        # PRIORIDAD 3: Formato antiguo con lista de huéspedes (PAXIMUM)
        elif 'huespedes' in data_reserva and data_reserva['huespedes']:
            for huesped in data_reserva['huespedes']:
                nombre_completo = f"{huesped.get('titulo', '')} {huesped.get('nombre', '')} {huesped.get('apellido', '')}".strip()
                if nombre_completo:
                    lista_pasajeros.append(nombre_completo)
            print(f"✅ PRIORIDAD 3: {len(lista_pasajeros)} pasajeros desde lista 'huespedes'")
        
        # Crear string de nombres
        if lista_pasajeros:
            nombres_clientes = ", ".join([str(p).upper() for p in lista_pasajeros])
            print(f"📝 Nombres finales ({len(lista_pasajeros)}): {nombres_clientes}")
        else:
            nombres_clientes = "HUÉSPED NO ESPECIFICADO"
            print("⚠️ Usando nombre genérico")
        
        # ===== 3. NACIONALIDAD - VERSIÓN ROBUSTA =====
        nacionalidad = 'NO ESPECIFICADA'
        
        # Verificar si hay nacionalidad en los datos
        if 'nacionalidad' in data_reserva and data_reserva['nacionalidad']:
            nacionalidad_raw = str(data_reserva['nacionalidad']).upper()
            print(f"🔍 Nacionalidad cruda recibida: '{nacionalidad_raw}'")
            
            # Limpiar: mantener letras, números y espacios
            nacionalidad_limpia = re.sub(r'[^A-Z0-9\s]', '', nacionalidad_raw).strip()
            
            if nacionalidad_limpia and len(nacionalidad_limpia) >= 2:
                nacionalidad = nacionalidad_limpia
                print(f"✅ Nacionalidad válida: {nacionalidad}")
            else:
                print(f"⚠️ Nacionalidad inválida: '{nacionalidad_limpia}'")
        
        # Si no hay nacionalidad específica, buscar en otras fuentes
        if nacionalidad == 'NO ESPECIFICADA' and 'huespedes' in data_reserva and data_reserva['huespedes']:
            # Tomar nacionalidad del primer huésped
            primer_huesped = data_reserva['huespedes'][0]
            if 'nacionalidad' in primer_huesped:
                nacionalidad = primer_huesped['nacionalidad'].upper()
                print(f"✅ Nacionalidad desde primer huésped: {nacionalidad}")
        
        # ===== 4. CONVERTIR DATOS DEL PARSER AL FORMATO ESPERADO =====
        # El parser de BedBankGlobal devuelve datos en formato nuevo
        # Necesitamos convertirlos al formato que espera el resto del código
        
        # Habitaciones: convertir formato nuevo a antiguo
        habitaciones_formato_antiguo = []
        if 'habitaciones' in data_reserva and data_reserva['habitaciones']:
            for hab in data_reserva['habitaciones']:
                habitacion_convertida = {
                    'tipo': hab.get('tipo', ''),
                    'adultos': hab.get('adultos', 2),
                    'ninos': hab.get('ninos', 0),
                    'bebes': hab.get('bebes', 0),
                    'regimen': data_reserva.get('meal_plan', 'ALL INCLUSIVE')
                }
                habitaciones_formato_antiguo.append(habitacion_convertida)
        else:
            # Si no hay habitaciones, crear una por defecto
            habitaciones_formato_antiguo = [{
                'tipo': 'HABITACIÓN STANDARD',
                'adultos': len(lista_pasajeros) if lista_pasajeros else 2,
                'ninos': 0,
                'bebes': 0,
                'regimen': data_reserva.get('meal_plan', 'ALOJAMIENTO DESAYUNO')
            }]
        ...
        print(f"💰 Precio final: {precio_venta}")
        
        # ===== 6. HOTEL =====
        hotel = data_reserva.get('hotel', '').upper()
        if not hotel or hotel == 'HOTEL NO ESPECIFICADO':
        ...
        # Múltiples formatos de fechas
        if 'fechas' in data_reserva and data_reserva['fechas']:
            date_from = data_reserva['fechas'].get('checkin')
            date_to = data_reserva['fechas'].get('checkout')
        elif 'fecha_entrada' in data_reserva and 'fecha_salida' in data_reserva:
            date_from = data_reserva['fecha_entrada']
            date_to = data_reserva['fecha_salida']
        
        if not date_from or not date_to:
            print(f"❌ Error: Fechas incompletas")
            return False
        
        # ===== 8. OBSERVACIONES =====
        num_habitaciones = len(habitaciones_formato_antiguo)
        num_pasajeros = len(lista_pasajeros)
        
        observaciones = f"IMPORTADO AUTOMÁTICAMENTE DESDE {fuente.upper()}. "
        observaciones += f"{num_habitaciones} HABITACIÓN(ES), {num_pasajeros} PASAJERO(S)."
        
        if nacionalidad != 'NO ESPECIFICADA':
            observaciones += f" NACIONALIDAD: {nacionalidad}."
        
        # ===== 9. CREAR LA RESERVA =====
        reserva = Reservation(
            status='OK',
            agency=fuente.upper(),
            booking_code=booking_voucher,
            clients_names=nombres_clientes,
            hotel=hotel,
            date_from=date_from,
            date_to=date_to,
        ...
        print(f"✅ Reserva guardada: {reserva.booking_code}")
        print(f"   Nombres ({num_pasajeros}): {nombres_clientes[:100]}...")
        print(f"   Nacionalidad: {nacionalidad}")
        print(f"   Hotel: {hotel}")
        print(f"   Fechas: {date_from} a {date_to}")
        print(f"   Precio: {precio_venta}")
        
        # ===== 10. CREAR HABITACIONES =====
        for hab in habitaciones_formato_antiguo:
            total_ninos = hab.get('ninos', 0) + hab.get('bebes', 0)
            
            room = Room(
                reservation=reserva,
                room_type=hab.get('tipo', 'HABITACIÓN STANDARD').upper(),
                pax_ad=hab.get('adultos', 2),
                pax_chd=total_ninos
            )
            room.save()
        
        print(f"✅ {len(habitaciones_formato_antiguo)} habitación(es) creada(s)")
        print(f"🎉 Reserva {fuente} creada exitosamente: {reserva.booking_code}")
        return True
        
    except IntegrityError as e:
        if 'UNIQUE constraint' in str(e):
            print(f"❌ Error: Booking code duplicado {data_reserva.get('voucher', 'N/A')}")
        else:
            print(f"❌ Error de integridad: {e}")
        return False
        
    except Exception as e:
        print(f"❌ Error creando reserva {fuente}: {e}")
        import traceback
...
def dashboard(request):
    ...
    context = {
        ...
        'avg_profit_percentage_manual': avg_profit_percentage_manual,
...
def export_dashboard_excel(request):
    ...
    try:
        ...
        for hotel in hotel_counts[:5]:
            ws_dashboard[f'G{current_hotel_row}'] = hotel['hotel'][:30]
    ...
    if request.method == 'POST' and request.FILES.get('archivo_paximum'):
        try:
            ...
            for data_reserva in reservas_data:
                if crear_reserva_desde_importacion(data_reserva, request.user, 'PAXIMUM'):
                    reservas_creadas += 1
...
def importar_bedbankglobal(request):
    """Importar reservas desde archivos HTML de BedBankGlobal"""
    if request.method == 'POST':
        if 'archivo_html' in request.FILES:
            ...
            try:
                ...
                # Procesar el archivo con el parser
                from .parser_bedbankglobal import parsear_bedbankglobal
                reservas = parsear_bedbankglobal(html_content)
                ...
                for i, reserva_data in enumerate(reservas, 1):
                    ...
                    try:
                        # ===== ADAPTACIÓN CORRECTA DE DATOS =====
                        # El parser devuelve datos en formato nuevo, los pasamos DIRECTAMENTE
                        # sin convertir al formato antiguo
                        
                        datos_adaptados = {
                            'voucher': reserva_data['booking_code'],
                            'booking_code': reserva_data['booking_code'],  # Nuevo campo
                            'hotel': reserva_data.get('hotel', '').upper(),
                            'fechas': {
                                'checkin': reserva_data['fecha_entrada'],
                                'checkout': reserva_data['fecha_salida']
                            },
                            'fecha_entrada': reserva_data['fecha_entrada'],  # Nuevo
                            'fecha_salida': reserva_data['fecha_salida'],    # Nuevo
                            # PASAJEROS EN FORMATO NUEVO (lista)
                            'pasajeros': reserva_data.get('pasajeros', []),
                            # NOMBRES COMPLETOS (string concatenado)
                            'nombres_completos': ", ".join(reserva_data.get('pasajeros', [])).upper(),
                            # Habitaciones en formato correcto
                            'habitaciones': [],
                            ...
                            'meal_plan': reserva_data.get('meal_plan', 'ALL INCLUSIVE').upper()
                        }
                        ...
                        print(f"✅ Datos adaptados - Pasajeros: {len(datos_adaptados['pasajeros'])}")
                        print(f"✅ Datos adaptados - Nombres completos: {datos_adaptados['nombres_completos']}")
                        print(f"✅ Datos adaptados - Nacionalidad: {datos_adaptados['nacionalidad']}")
                        
                        # Usar la función genérica para crear la reserva
                        if crear_reserva_desde_importacion(datos_adaptados, request.user, 'BEDBANKGLOBAL'):
                            reservas_creadas += 1
